...
void printLocalTime()
{
  ...
  strftime(dati, 20, "%d/%m/%Y %H:%M:%S", &timeinfo);
  Serial.println(dati);
}
...
void loop() {
  ...
  if (meteo == true) {
// Download Cumulus HWS template https://github.com/ktrue/CU-HWS
    while (1) {
      M5.update();
      if (millis() - Tempo_2 == 15000) {
        Tempo_2 = millis();
        ftp.OpenConnection();
        ftp.ChangeWorkDir("/meteo");
        printLocalTime();
        ftp.InitFile("Type A");
        ftp.NewFile("realtime.txt");
        dati_file.concat(dati);
        Serial.println("dati_file = " + dati_file);
        for (int i = 0; i < 15; i++) {
        ...
        somma /= 15;
        somma_1 /= 15;
        Serial.println("Temp = " + String(somma / 100 - 11));
        dati_file += " " + String(somma / 100 - 11);
        Serial.println("Press = " + String(somma_1));
        dati_file += "        " + String(somma_1) + " NW 0 km/h C hPa mm      " + String(somma / 100 - 11);
        dati_file.toCharArray(dati, 128);    // -----> strcat(dest, src);
        ftp.Write(dati);  //"15/12/22 11:46:32 28.8 60 12.7 0 1 315 0.0 0.0 1019.3 NW 0 km/h C hPa mm 27.0 +0.2 28.5 631.8 0.9 22.5 73 20.8 +1.4 20.8 11:44 12.0 02:39 13 01:42 23 01:42 1020.0 11:12 1015.8 00:00 3.13.6 3152 4 20.8 23.4 0.0 0.00 0 289 0.0 1 1 0 WNW 3293 ft 21.7 0.0 955 0 21.9");
        ftp.CloseFile();
        ftp.CloseConnection();
        somma = 0;
        somma_1 = 0;
        ...
        case 5:
          ...
          if (M5.BtnB.wasReleased()) {
            ...
            while (1) {
              ...
              Serial.println("You want to change the type of bluetooth end restart? Button B to yes or button C to no");
